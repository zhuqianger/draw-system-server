...
CREATE TABLE IF NOT EXISTS `auction` (
    ...
    `startTime` DATETIME NULL COMMENT '拍卖开始时间（管理员点击开始后设置）',
    `endTime` DATETIME NULL COMMENT '拍卖结束时间',
    `duration` INT NULL COMMENT '拍卖时长（秒）',
    `status` VARCHAR(20) NOT NULL DEFAULT 'WAITING' COMMENT '状态：WAITING-等待开始, FIRST_PHASE-第一阶段, PICKUP_PHASE-捡漏环节, FINISHED-已结束, CANCELLED-已取消',
    `phase` VARCHAR(20) NULL COMMENT '阶段：WAITING-等待开始, FIRST_PHASE-第一阶段(30s), PICKUP_PHASE-捡漏环节(30s)',
    `startingPrice` DECIMAL(10,2) NULL COMMENT '起拍价',
    `maxPrice` DECIMAL(10,2) NULL COMMENT '最高出价（基础定价+2.5）',
